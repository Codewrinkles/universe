...
#pragma warning restore CS8618
    ...
    /// <summary>
    /// Creates a job for ingesting an article.
    /// </summary>
    public static ContentIngestionJob CreateForArticle(
        string title,
        string? author = null,
        string? url = null)
    {
        ...
        return new ContentIngestionJob
        {
            ...
            Status = IngestionJobStatus.Queued,
            Title = Truncate(title.Trim(), MaxTitleLength),
            ParentDocumentId = $"article_{Guid.NewGuid():N}",
            Author = author?.Trim() is { Length: > 0 } a ? Truncate(a, MaxAuthorLength) : null,
            SourceUrl = url?.Trim() is { Length: > 0 } u ? Truncate(u, MaxUrlLength) : null,
            CreatedAt = DateTimeOffset.UtcNow
        };
    }
