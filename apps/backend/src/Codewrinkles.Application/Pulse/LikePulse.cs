    ...
    : ICommandHandler<LikePulseCommand, LikePulseResult>
{
    ...
    {
        using var activity = TelemetryExtensions.StartApplicationActivity(SpanNames.Engagement.Like);
        ...
        try
        {
            ...
            try
            {
                ...
                if (pulse is not null && pulse.AuthorId != command.ProfileId)
                {
                    ...
                    await _unitOfWork.SaveChangesAsync(cancellationToken);
                    AppMetrics.RecordNotificationCreated("like");
                }
