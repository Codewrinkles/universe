...
public sealed class ExtractMemoriesCommandHandler
    : ICommandHandler<ExtractMemoriesCommand, ExtractMemoriesResult>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILlmService _llmService;
    private readonly IEmbeddingService _embeddingService;

    public ExtractMemoriesCommandHandler(
    ...
    public async Task<ExtractMemoriesResult> HandleAsync(
        ExtractMemoriesCommand command,
        CancellationToken cancellationToken)
    {
        using var activity = TelemetryExtensions.StartApplicationActivity(SpanNames.Nova.ExtractMemories);
        ...
        try
        {
            ...
            {
                return new ExtractMemoriesResult(0, 0, 0);
            }
            ...
            if (extractedMemories.Count == 0)
            {
                ...
                session.MarkMemoryExtracted(lastMessageId);
            ...
            session.MarkMemoryExtracted(lastProcessedMessageId);

            await _unitOfWork.SaveChangesAsync(cancellationToken);

            activity?.SetSuccess(true);
            activity?.SetTag("memories.created", memoriesCreated);
            activity?.SetTag("memories.superseded", memoriesSuperseded);
            activity?.SetTag("memories.reinforced", memoriesReinforced);

            return new ExtractMemoriesResult(
                memoriesCreated,
                memoriesSuperseded,
                memoriesReinforced);
        }
                ...
                focus.ValueKind == JsonValueKind.String)
            {
                ...
                if (!string.IsNullOrWhiteSpace(text))
                {
                    ...
                    memories.Add(new ExtractedMemory(
                        ...
                        text,
                        Math.Max(importance, 4))); // Current focus is always high importance
                }
            }
        }
        catch (JsonException)
