// <auto-generated />
using System;
using Codewrinkles.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Codewrinkles.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20251222175159_AddContentIngestionForRAG")]
    partial class AddContentIngestionForRAG
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Codewrinkles.Domain.Identity.ExternalLogin", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AccessToken")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<Guid>("IdentityId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Provider")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("ProviderAvatarUrl")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("ProviderDisplayName")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("ProviderEmail")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("ProviderUserId")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("RefreshToken")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset?>("TokenExpiresAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.HasKey("Id");

                    b.HasIndex("IdentityId")
                        .HasDatabaseName("IX_ExternalLogins_IdentityId");

                    b.HasIndex("Provider", "ProviderUserId")
                        .IsUnique()
                        .HasDatabaseName("UQ_ExternalLogins_Provider_UserId");

                    b.ToTable("ExternalLogins", "identity");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.Identity", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("EmailNormalized")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("FailedLoginAttempts")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsEmailVerified")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("LastLoginAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset?>("LockedUntil")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("PasswordHash")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("Role")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.HasKey("Id");

                    b.HasIndex("CreatedAt")
                        .HasDatabaseName("IX_Identities_CreatedAt");

                    b.HasIndex("EmailNormalized")
                        .IsUnique()
                        .HasDatabaseName("IX_Identities_EmailNormalized");

                    b.HasIndex("IsActive")
                        .HasDatabaseName("IX_Identities_IsActive");

                    b.ToTable("Identities", "identity");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.Profile", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AvatarUrl")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<string>("Bio")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("Handle")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid>("IdentityId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsFoundingMember")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Location")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("NovaAccess")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<bool>("OnboardingCompleted")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("WebsiteUrl")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.HasKey("Id");

                    b.HasIndex("Handle")
                        .IsUnique()
                        .HasDatabaseName("IX_Profiles_Handle")
                        .HasFilter("[Handle] IS NOT NULL");

                    b.HasIndex("IdentityId")
                        .IsUnique()
                        .HasDatabaseName("IX_Profiles_IdentityId");

                    b.ToTable("Profiles", "identity");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.RefreshToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset>("ExpiresAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("IdentityId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsRevoked")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsUsed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid?>("ReplacedByTokenId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset?>("RevokedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("RevokedReason")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.HasIndex("ExpiresAt");

                    b.HasIndex("ReplacedByTokenId");

                    b.HasIndex("TokenHash")
                        .IsUnique();

                    b.HasIndex("IdentityId", "IsUsed", "IsRevoked", "ExpiresAt");

                    b.ToTable("RefreshTokens", "identity");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Notification.Notification", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ActorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid?>("EntityId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("EntityType")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<bool>("IsRead")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("ReadAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("RecipientId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<byte>("Type")
                        .HasColumnType("tinyint");

                    b.HasKey("Id");

                    b.HasIndex("ActorId");

                    b.HasIndex("RecipientId", "CreatedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_Notifications_RecipientId_CreatedAt");

                    b.HasIndex("RecipientId", "IsRead")
                        .HasDatabaseName("IX_Notifications_RecipientId_IsRead_Unread")
                        .HasFilter("[IsRead] = 0");

                    b.ToTable("Notifications", "notification");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.AlphaApplication", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("Goal")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<string>("InviteCode")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<bool>("InviteCodeRedeemed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("PrimaryTechStack")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<DateTimeOffset?>("RedeemedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid?>("RedeemedByProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<int>("Status")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<int>("YearsOfExperience")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .HasDatabaseName("IX_AlphaApplications_Email");

                    b.HasIndex("InviteCode")
                        .IsUnique()
                        .HasDatabaseName("IX_AlphaApplications_InviteCode")
                        .HasFilter("[InviteCode] IS NOT NULL");

                    b.HasIndex("Status")
                        .HasDatabaseName("IX_AlphaApplications_Status");

                    b.ToTable("AlphaApplications", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.ContentChunk", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Author")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int?>("ChunkIndex")
                        .HasColumnType("int");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(8000)
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<byte[]>("Embedding")
                        .IsRequired()
                        .HasColumnType("varbinary(max)");

                    b.Property<TimeSpan?>("EndTime")
                        .HasColumnType("time");

                    b.Property<string>("ParentDocumentId")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTimeOffset?>("PublishedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("SectionPath")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("SourceIdentifier")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<TimeSpan?>("StartTime")
                        .HasColumnType("time");

                    b.Property<string>("Technology")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("TokenCount")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("Author")
                        .HasDatabaseName("IX_ContentChunks_Author");

                    b.HasIndex("ParentDocumentId")
                        .HasDatabaseName("IX_ContentChunks_ParentDocumentId");

                    b.HasIndex("Source")
                        .HasDatabaseName("IX_ContentChunks_Source");

                    b.HasIndex("Technology")
                        .HasDatabaseName("IX_ContentChunks_Technology");

                    b.HasIndex("Source", "SourceIdentifier")
                        .IsUnique()
                        .HasDatabaseName("IX_ContentChunks_Source_SourceIdentifier");

                    b.ToTable("ContentChunks", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.ContentIngestionJob", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Author")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int>("ChunksCreated")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("ErrorMessage")
                        .HasMaxLength(4000)
                        .HasColumnType("nvarchar(4000)");

                    b.Property<int?>("MaxPages")
                        .HasColumnType("int");

                    b.Property<int?>("PagesProcessed")
                        .HasColumnType("int");

                    b.Property<string>("ParentDocumentId")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("SourceUrl")
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<DateTimeOffset?>("StartedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("Technology")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int?>("TotalPages")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CreatedAt")
                        .HasDatabaseName("IX_ContentIngestionJobs_CreatedAt");

                    b.HasIndex("ParentDocumentId")
                        .HasDatabaseName("IX_ContentIngestionJobs_ParentDocumentId");

                    b.HasIndex("Status")
                        .HasDatabaseName("IX_ContentIngestionJobs_Status");

                    b.ToTable("ContentIngestionJobs", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.ConversationSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<DateTimeOffset?>("LastMemoryExtractionAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset>("LastMessageAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<Guid?>("LastProcessedMessageId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Title")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("IX_ConversationSessions_IsDeleted");

                    b.HasIndex("ProfileId")
                        .HasDatabaseName("IX_ConversationSessions_ProfileId");

                    b.HasIndex("ProfileId", "LastMemoryExtractionAt")
                        .HasDatabaseName("IX_ConversationSessions_ProfileId_LastMemoryExtractionAt");

                    b.HasIndex("ProfileId", "IsDeleted", "LastMessageAt")
                        .IsDescending(false, false, true)
                        .HasDatabaseName("IX_ConversationSessions_ProfileId_IsDeleted_LastMessageAt");

                    b.ToTable("ConversationSessions", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.LearnerProfile", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("CurrentProject")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("CurrentRole")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<int?>("ExperienceYears")
                        .HasColumnType("int");

                    b.Property<string>("IdentifiedStrengths")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<string>("IdentifiedStruggles")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<string>("LearningGoals")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<string>("LearningStyle")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("PreferredPace")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("PrimaryTechStack")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("ProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.HasKey("Id");

                    b.HasIndex("ProfileId")
                        .IsUnique()
                        .HasDatabaseName("IX_LearnerProfiles_ProfileId");

                    b.ToTable("LearnerProfiles", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.Memory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<byte[]>("Embedding")
                        .HasColumnType("varbinary(max)");

                    b.Property<int>("Importance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(3);

                    b.Property<int>("OccurrenceCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.Property<Guid>("ProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("SourceSessionId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset?>("SupersededAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid?>("SupersededById")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("SourceSessionId")
                        .HasDatabaseName("IX_Memories_SourceSessionId");

                    b.HasIndex("SupersededById");

                    b.HasIndex("ProfileId", "CreatedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_Memories_ProfileId_CreatedAt_Desc");

                    b.HasIndex("ProfileId", "Importance")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_Memories_ProfileId_Importance_Desc");

                    b.HasIndex("ProfileId", "Category", "SupersededAt")
                        .HasDatabaseName("IX_Memories_ProfileId_Category_SupersededAt")
                        .HasFilter("[SupersededAt] IS NULL");

                    b.ToTable("Memories", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.Message", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(100000)
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<string>("ModelUsed")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<byte>("Role")
                        .HasColumnType("tinyint");

                    b.Property<Guid>("SessionId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int?>("TokensUsed")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("SessionId", "CreatedAt")
                        .HasDatabaseName("IX_Messages_SessionId_CreatedAt");

                    b.ToTable("Messages", "nova");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.Hashtag", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset>("LastUsedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<int>("PulseCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<string>("Tag")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("TagDisplay")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("Tag")
                        .IsUnique()
                        .HasDatabaseName("UQ_Hashtags_Tag");

                    b.HasIndex("PulseCount", "LastUsedAt")
                        .HasDatabaseName("IX_Hashtags_PulseCount_LastUsedAt");

                    b.ToTable("Hashtags", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.Pulse", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("AuthorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetimeoffset")
                        .HasDefaultValueSql("SYSDATETIMEOFFSET()");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid?>("ParentPulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("RepulsedPulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("ThreadRootId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<byte>("Type")
                        .HasColumnType("tinyint");

                    b.Property<DateTimeOffset?>("UpdatedAt")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("AuthorId")
                        .HasDatabaseName("IX_Pulses_AuthorId");

                    b.HasIndex("CreatedAt")
                        .IsDescending()
                        .HasDatabaseName("IX_Pulses_CreatedAt");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("IX_Pulses_IsDeleted");

                    b.HasIndex("ParentPulseId")
                        .HasDatabaseName("IX_Pulses_ParentPulseId")
                        .HasFilter("[ParentPulseId] IS NOT NULL");

                    b.HasIndex("RepulsedPulseId")
                        .HasDatabaseName("IX_Pulses_RepulsedPulseId")
                        .HasFilter("[RepulsedPulseId] IS NOT NULL");

                    b.HasIndex("ThreadRootId")
                        .HasDatabaseName("IX_Pulses_ThreadRootId")
                        .HasFilter("[ThreadRootId] IS NOT NULL");

                    b.HasIndex("IsDeleted", "CreatedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_Pulses_IsDeleted_CreatedAt");

                    b.HasIndex("AuthorId", "IsDeleted", "CreatedAt")
                        .IsDescending(false, false, true)
                        .HasDatabaseName("IX_Pulses_AuthorId_IsDeleted_CreatedAt");

                    b.ToTable("Pulses", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseBookmark", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("ProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("PulseId");

                    b.HasIndex("ProfileId", "CreatedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_PulseBookmarks_ProfileId_CreatedAt");

                    b.HasIndex("ProfileId", "PulseId")
                        .IsUnique()
                        .HasDatabaseName("IX_PulseBookmarks_ProfileId_PulseId");

                    b.ToTable("PulseBookmarks", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseEngagement", b =>
                {
                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("LikeCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<int>("ReplyCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<int>("RepulseCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<long>("ViewCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasDefaultValue(0L);

                    b.HasKey("PulseId");

                    b.ToTable("PulseEngagements", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseHashtag", b =>
                {
                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("HashtagId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<int>("Position")
                        .HasColumnType("int");

                    b.HasKey("PulseId", "HashtagId");

                    b.HasIndex("HashtagId")
                        .HasDatabaseName("IX_PulseHashtags_HashtagId");

                    b.ToTable("PulseHashtags", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AltText")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("Height")
                        .HasColumnType("int");

                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("nvarchar(2048)");

                    b.Property<int>("Width")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PulseId")
                        .IsUnique()
                        .HasDatabaseName("IX_PulseImages_PulseId");

                    b.ToTable("PulseImages", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseLike", b =>
                {
                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("PulseId", "ProfileId");

                    b.HasIndex("CreatedAt")
                        .HasDatabaseName("IX_PulseLikes_CreatedAt");

                    b.HasIndex("ProfileId")
                        .HasDatabaseName("IX_PulseLikes_ProfileId");

                    b.ToTable("PulseLikes", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseLinkPreview", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Description")
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<string>("Domain")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("ImageUrl")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.HasKey("Id");

                    b.HasIndex("PulseId")
                        .IsUnique()
                        .HasDatabaseName("IX_PulseLinkPreviews_PulseId");

                    b.ToTable("PulseLinkPreviews", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseMention", b =>
                {
                    b.Property<Guid>("PulseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ProfileId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Handle")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("PulseId", "ProfileId");

                    b.HasIndex("CreatedAt");

                    b.HasIndex("Handle");

                    b.HasIndex("ProfileId");

                    b.ToTable("PulseMentions", "pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Social.Follow", b =>
                {
                    b.Property<Guid>("FollowerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("FollowingId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("FollowerId", "FollowingId");

                    b.HasIndex("FollowerId")
                        .HasDatabaseName("IX_Follows_FollowerId_FollowingId");

                    SqlServerIndexBuilderExtensions.IncludeProperties(b.HasIndex("FollowerId"), new[] { "FollowingId", "CreatedAt" });

                    b.HasIndex("FollowingId")
                        .HasDatabaseName("IX_Follows_FollowingId_FollowerId");

                    SqlServerIndexBuilderExtensions.IncludeProperties(b.HasIndex("FollowingId"), new[] { "FollowerId", "CreatedAt" });

                    b.ToTable("Follows", "social");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.ExternalLogin", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Identity", "Identity")
                        .WithMany("ExternalLogins")
                        .HasForeignKey("IdentityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Identity");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.Profile", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Identity", "Identity")
                        .WithOne("Profile")
                        .HasForeignKey("Codewrinkles.Domain.Identity.Profile", "IdentityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Identity");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.RefreshToken", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Identity", "Identity")
                        .WithMany()
                        .HasForeignKey("IdentityId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Identity.RefreshToken", "ReplacedByToken")
                        .WithMany()
                        .HasForeignKey("ReplacedByTokenId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("Identity");

                    b.Navigation("ReplacedByToken");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Notification.Notification", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Actor")
                        .WithMany()
                        .HasForeignKey("ActorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Recipient")
                        .WithMany()
                        .HasForeignKey("RecipientId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Actor");

                    b.Navigation("Recipient");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.ConversationSession", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Owner")
                        .WithMany()
                        .HasForeignKey("ProfileId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.LearnerProfile", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Profile")
                        .WithOne()
                        .HasForeignKey("Codewrinkles.Domain.Nova.LearnerProfile", "ProfileId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Profile");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.Memory", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Profile")
                        .WithMany()
                        .HasForeignKey("ProfileId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Nova.ConversationSession", "SourceSession")
                        .WithMany()
                        .HasForeignKey("SourceSessionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Nova.Memory", "SupersededBy")
                        .WithMany()
                        .HasForeignKey("SupersededById")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("Profile");

                    b.Navigation("SourceSession");

                    b.Navigation("SupersededBy");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.Message", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Nova.ConversationSession", "Session")
                        .WithMany("Messages")
                        .HasForeignKey("SessionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Session");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.Pulse", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Author")
                        .WithMany()
                        .HasForeignKey("AuthorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "ParentPulse")
                        .WithMany()
                        .HasForeignKey("ParentPulseId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "RepulsedPulse")
                        .WithMany()
                        .HasForeignKey("RepulsedPulseId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "ThreadRoot")
                        .WithMany()
                        .HasForeignKey("ThreadRootId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Author");

                    b.Navigation("ParentPulse");

                    b.Navigation("RepulsedPulse");

                    b.Navigation("ThreadRoot");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseBookmark", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Profile")
                        .WithMany()
                        .HasForeignKey("ProfileId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithMany()
                        .HasForeignKey("PulseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Profile");

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseEngagement", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithOne("Engagement")
                        .HasForeignKey("Codewrinkles.Domain.Pulse.PulseEngagement", "PulseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseHashtag", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Pulse.Hashtag", "Hashtag")
                        .WithMany("PulseHashtags")
                        .HasForeignKey("HashtagId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithMany()
                        .HasForeignKey("PulseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Hashtag");

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseImage", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithOne("Image")
                        .HasForeignKey("Codewrinkles.Domain.Pulse.PulseImage", "PulseId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseLike", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Profile")
                        .WithMany()
                        .HasForeignKey("ProfileId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithMany()
                        .HasForeignKey("PulseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Profile");

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseLinkPreview", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithOne("LinkPreview")
                        .HasForeignKey("Codewrinkles.Domain.Pulse.PulseLinkPreview", "PulseId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.PulseMention", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "MentionedProfile")
                        .WithMany()
                        .HasForeignKey("ProfileId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Pulse.Pulse", "Pulse")
                        .WithMany()
                        .HasForeignKey("PulseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MentionedProfile");

                    b.Navigation("Pulse");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Social.Follow", b =>
                {
                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Follower")
                        .WithMany()
                        .HasForeignKey("FollowerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Codewrinkles.Domain.Identity.Profile", "Following")
                        .WithMany()
                        .HasForeignKey("FollowingId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Follower");

                    b.Navigation("Following");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Identity.Identity", b =>
                {
                    b.Navigation("ExternalLogins");

                    b.Navigation("Profile")
                        .IsRequired();
                });

            modelBuilder.Entity("Codewrinkles.Domain.Nova.ConversationSession", b =>
                {
                    b.Navigation("Messages");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.Hashtag", b =>
                {
                    b.Navigation("PulseHashtags");
                });

            modelBuilder.Entity("Codewrinkles.Domain.Pulse.Pulse", b =>
                {
                    b.Navigation("Engagement")
                        .IsRequired();

                    b.Navigation("Image");

                    b.Navigation("LinkPreview");
                });
#pragma warning restore 612, 618
        }
    }
}
